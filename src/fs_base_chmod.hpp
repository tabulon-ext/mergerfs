...
namespace fs
{
  namespace base
  {
    ...
    {
      return fs::base::fchmod(fd_,st_->st_mode);
    }
      ...
      if(rv == -1)
        {
          ...
          error = errno;
          rv = fs::base::stat(path_,&tmpst);
          if(rv == -1)
            return -1;

          if((tmpst.st_mode & MODE_BITS) != (mode & MODE_BITS))
